...
import { Navbar, PageHeroBanner, Footer } from "@/Components/organisms";
import { NAV_LINKS, FOOTER_LINKS } from "@/constants/welcomeData";
import { KUB_TALK_GALLERY } from "@/constants/kubTalkData";

/**
 * Molecule: GalleryCard
 * A single image card with title and description overlay on hover.
 */
function GalleryCard({ image, title, desc }) {
    return (
        <div className="group relative rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 cursor-pointer">
            ...
            {/* Dark overlay — always slightly present, stronger on hover */}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                ...
                <p className="text-white/80 text-xs leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
                    {desc}
                </p>
            </div>
        </div>
 ...
 * Page: KUB Talk
 *
 * Image gallery page documenting KUB Talk events with a 3-column grid layout.
 */
            ...
            <div className="min-h-screen bg-white font-sans antialiased">
                    ...
                    <div className="max-w-7xl mx-auto px-6 lg:px-8">
                        ...
                        <div className="mb-10">
                            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">
                                <span className="text-transparent bg-clip-text bg-linear-to-r from-[#6B1B1B] to-[#C4571A]">
                                    KUB Talk
                                </span>{" "}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {KUB_TALK_GALLERY.map((item, index) => (
                                <div
                                    key={index}
                ...
                {lightbox !== null && (
                    ...
                    >
                        ...
                        >
                            <img
                                src={KUB_TALK_GALLERY[lightbox].image}
                                alt={KUB_TALK_GALLERY[lightbox].title}
                                className="w-full max-h-[65vh] object-cover"
                            />
                            <div className="p-5">
                                <h3 className="text-lg font-bold text-gray-900">
                                    {KUB_TALK_GALLERY[lightbox].title}
                                </h3>
                                <p className="mt-1 text-sm text-gray-500">
                                    {KUB_TALK_GALLERY[lightbox].desc}
                                </p>
                            </div>
                            ...
                            {lightbox < KUB_TALK_GALLERY.length - 1 && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        setLightbox(lightbox + 1);
